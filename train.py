...
SRC_TRAIN_DATA = "combine_train_idx.en"       
TRG_TRAIN_DATA = "combine_train_idx.zh"        
CHECKPOINT_PATH = "nmt_ckpt/seq2seq_ckpt"   
...
BATCH_SIZE = 128                     # 训练数据batch的大小。
EMBED_SIZE = 256                      #embedding size can be different from hidden size
NUM_EPOCH = 16                        # 使用训练数据的轮数。
...
def MakeSrcTrgDataset(src_path, trg_path, batch_size):
    ...
    # 随机打乱训练数据。
    dataset = dataset.shuffle(990000)
...
class NMTModel(object):
    ...
    def forward(self, src_input, src_size, trg_input, trg_label, trg_size):
        ...
        # flat logits and labels and label weights for easier computation
        ...
        # tf.train_variables(): returns all defined trainable variables,
        # it serves as xs here 
        ...
        # use the clipped gradients for minimizing
...
def run_epoch(session, cost_op, train_op, saver, step, merge, writer):
    # 训练一个epoch。
    # 重复训练步骤直至遍历完Dataset中所有数据。
    while True:
        try:
            # 运行train_op并计算损失值。训练数据在main()函数中以Dataset方式提供。
            cost, _, result = session.run([cost_op, train_op, merge])
            writer.add_summary(result, step)
            if step % 20 == 0:
                logger.info("After %d steps, per token cost is %.3f" % (step, cost))
            # 每200步保存一个checkpoint。
            if step % 200 == 0:
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(NUM_EPOCH):
            ...
            step = run_epoch(sess, cost_op, train_op, saver, step, merge, writer)
